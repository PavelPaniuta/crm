...
| Задачи | `TasksTab`, `lib/tasks.ts` |
| Чат | `ChatTab`, `lib/chat.ts` |
| AI ассистент | `AssistantTab`, `lib/assistant.ts` |
| Модалка сделки (state) | `hooks/useDealForm.ts` |

`page.tsx` сокращён с ~5k до ~3k строк. В монолите остаются: настройки, зарплата, сотрудники, профиль, shell/навигация.

## Дальше (по желанию)

1. Вынести вкладку «Настройки» (статусы клиентов, поля, организации)
2. Вынести «Зарплата» / детали сотрудника
3. Общий layout: sidebar + tab routing (`AppShell`)
