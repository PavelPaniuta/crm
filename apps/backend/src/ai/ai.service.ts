...
7. КЛИЕНТЫ: если в сообщении пользователя есть и имя (или строка «Клиент:» / «🧍»), и телефон (или «Телефон:» / «☎️») — НЕ отвечай только текстом с пересказом. Обязательно вызови tool confirm_create_client с извлечёнными полями. То же для вставки уведомления о новом звонке (Telegram/бот: «Новый звонок», банк, ассистент, summary, время).
8. Время начала из текста вида «04.05.2026, 11:31» переводи в callStartedAt как ISO 8601 (например 2026-05-04T11:31:00).
9. Если пользователь явно просит «создай клиента» и дал имя + телефон — сразу confirm_create_client; statusSlug по умолчанию "new".`;
    ...
    const tools: ChatCompletionTool[] = [
      ...
      },
      {
        ...
        function: {
          ...
          description:
            'Подготовить карточку клиента к подтверждению. Вызывай ВСЕГДА, когда из сообщения можно извлечь имя клиента и номер телефона (в т.ч. вставка из бота/Telegram). Не заменяй этот tool длинным текстовым ответом.',
          parameters: {
            type: 'object',
            properties: {
