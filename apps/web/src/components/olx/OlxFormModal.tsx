...
type Props = {
  ...
  form: OlxFormState;
  setForm: React.Dispatch<React.SetStateAction<OlxFormState>>;
  onClose: () => void;
  onSave: () => void | Promise<void>;
};

export function OlxFormModal({ open, editingId, form, setForm, onClose, onSave }: Props) {
  if (!open) return null;
  return (
    <div
    ...
    >
      <div className="card" style={{ width: 440, maxWidth: "100%" }} onMouseDown={(e) => e.stopPropagation()}>
        <div className="card-header">
          <span className="card-title">{editingId ? "Редактировать ОЛХ" : "Новый ОЛХ"}</span>
        </div>
